...
if __name__ == "__main__":
    ...
    for i in range(0, len(edge_list)):
        ...
        edge_number = int(i)
        #current_new_edge_list = new_edge_series[edge_number]
        current_edge = edge_list[edge_number]
        print(current_edge)

        #Removal of edges that falls within their compatible sequence blocks
        remove_nonviable_edge = CheckEdgeList()
        exclude_edge = remove_nonviable_edge.RemovNonViableEdges(current_edge, sequence_blocks)
    ...
    print('New sequence block list written to ', output_filename)
